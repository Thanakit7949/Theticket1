...
const TicketSport: React.FC = () => {
  ...
  const { price, label, selectedSeats, sport_Zone } = location.state || {}; // ดึงข้อมูลจาก state
  ...
  const handleDownloadImage = async () => {
    if (ticketRef.current) {
      setLoading(true);
      await new Promise((resolve) => setTimeout(resolve, 500)); // รอให้ DOM render เสร็จ
      try {
  ...
  useEffect(() => {
    if (!location.state) {
      ...
      console.log("sport_Zone.sportid:", sport_Zone?.sportid);
      console.log("detail:", detail);

      const fetchBookingDetails = async () => {
        let userId = Cookies.get("userid");
        let sportId = sport_Zone?.sportid; // ใช้ `?.` ป้องกัน undefined error

        if (!userId || !sportId) {
          console.warn("user_id หรือ sport_id ไม่มีค่า", {
            userId,
            sportId,
          });
          return;
        ...
        try {
          const response = await fetch(
            `http://localhost:5000/api/bookingSport-details?user_id=${userId}&sport_id=${sportId}`
          );
          if (!response.ok) {
  ...
  }, [location.state, selectedSeats, sport_Zone]);
  ...
  const groupedBookings =
    datadetail?.reduce(
      (acc, item: any) => {
        const key = `${item.sport_name}_${item.sport_location}_${item.sport_date}_${item.sport_time}_${item.sport_img}`;

        if (!acc[key]) {
          acc[key] = {
            sport_img: `http://localhost/sport/all/${item.sport_img}`,
            sport_name: item.sport_name,
            sport_location: item.sport_location,
            sport_date: item.sport_date,
            sport_time: item.sport_time,
    ...
    <section style={{ textAlign: "center", padding: 20 }}>
      ...
      >
        ...
        <Box sx={{ flex: "1 1 auto", mr: 3 }}>
          {/* <img
          src={formattedData[0]?.sport_img}
          alt={formattedData[0]?.sport_name}
        ...
        <Box sx={{ color: "#151515" }}>
          ...
          <Box>
            <Typography variant="h5" color="black" fontWeight="bold">
              {formattedData[0]?.sport_name || "N/A"}
            </Typography>
            <Box
              display="flex"
            ...
            >
              ...
              <Typography variant="h6">
                {formattedData[0]?.sport_date
                  ? new Date(formattedData[0].sport_date).toLocaleDateString(
                      "th-TH",
                      {
                        year: "numeric",
                        month: "2-digit",
            ...
            >
              ...
              <Typography variant="h6">
                {formattedData[0]?.sport_time || "N/A"}
              </Typography>
            </Box>
            ...
            >
              ...
              <Typography variant="h6">
                {formattedData[0]?.sport_location || "N/A"}
              </Typography>
            </Box>
            ...
            >
              ...
              <Typography
                variant="h6"
                sx={{ textAlign: "right", color: "#ec407a" }}
              >
